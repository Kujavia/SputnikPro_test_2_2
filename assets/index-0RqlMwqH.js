            ...
            <body>
              ...
              <body>
                ...
                <body>
                    <table>
                        <thead>
                            <tr>`),Object.keys(l.data[0]).forEach(c=>{i.write(`<th>${c}</th>`)}),i.write("</tr></thead><tbody>"),l.data.forEach(c=>{i.write("<tr>"),Object.values(c).forEach(p=>{i.write(`<td>${p}</td>`)}),i.write("</tr>")}),i.write("</tbody></table>"),i.write("</body></html>"),i.close()}})};function O(){document.getElementById("summaryPlan_modal").style.display="flex"}function D(){document.getElementById("summaryPlan_modal").style.display="none"}function N(){const e=document.querySelectorAll('#summaryPlan_modalContent input[type="checkbox"]'),t=[];e.forEach(o=>{o.checked&&t.push(o.name)})}window.summaryPlan_OptimizationButton=O;window.summaryPlan_closeModalButton=D;window.summaryPlan_saveModalButton=N;window.summaryPlan_PreviewButton=function(){const t=document.getElementById("summary_plan_time").value,n=document.getElementById("summary_plan_geography").value,r=document.getElementById("summary_plan_scu").value,m=document.getElementById("summary_plan_method").value;let u;if(t!=="not_selected"&&n!=="not_selected"&&r!=="not_selected"&&m!=="not_selected")u="./images/demo_file/test_forecast_csv.csv";else if(t==="not_selected"||n==="not_selected"||r==="not_selected"||m==="not_selected"){const l=document.getElementById("summaryPlan_PreviewIframe"),s=l.contentDocument||l.contentWindow.document;s.open(),s.write(`<html>
              <head>
                  <title>Сообщение</title>
                  <style>
                      body {
