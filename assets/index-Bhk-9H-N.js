            ...
            <body>
              ...
              <body>
                ...
                <body>
                    <table>
                        <thead>
                            <tr>`),Object.keys(i.data[0]).forEach(h=>{l.write(`<th>${h}</th>`)}),l.write("</tr></thead><tbody>"),i.data.forEach(h=>{l.write("<tr>"),Object.values(h).forEach(f=>{l.write(`<td>${f}</td>`)}),l.write("</tr>")}),l.write("</tbody></table>"),l.write("</body></html>"),l.close()}})};function A(){document.getElementById("summaryPlan_modal").style.display="flex"}function O(){document.getElementById("summaryPlan_modal").style.display="none"}function L(){const e=document.querySelectorAll('#summaryPlan_modalContent input[type="checkbox"]'),t=[];e.forEach(o=>{o.checked&&t.push(o.name)})}window.summaryPlan_OptimizationButton=A;window.summaryPlan_closeModalButton=O;window.summaryPlan_saveModalButton=L;window.summaryPlan_PreviewButton=function(){const t=document.getElementById("summary_plan_time").value,a=document.getElementById("summary_plan_geography").value,n=document.getElementById("summary_plan_scu").value,c=document.getElementById("summary_plan_method").value;let s;if(t!=="not_selected"&&a!=="not_selected"&&n!=="not_selected"&&c!=="not_selected")s="./images/demo_file/test_forecast_csv.csv";else if(t==="not_selected"||a==="not_selected"||n==="not_selected"||c==="not_selected"){const i=document.getElementById("summaryPlan_PreviewIframe"),d=i.contentDocument||i.contentWindow.document;d.open(),d.write(`<html>
              <head>
                  <title>Сообщение</title>
                  <style>
                      body {
